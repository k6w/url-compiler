...
describe("safe normalization", async () => {
  ...
  test("preserves bare flags and empty values distinctly", async () => {
  ...
  test("decodes percent-encoded unreserved characters", async () => {
  ...
  test("keeps reserved percent-escapes intact", async () => {
  ...
  test("bare query and fragment markers survive round-trip", async () => {
    expect(canonicalize("https://example.com/?").canonical).toBe("https://example.com/?")
    expect(canonicalize("https://example.com/#").canonical).toBe("https://example.com/#")
    expect(canonicalize("https://example.com/?#").canonical).toBe("https://example.com/?#")
  })

  test("internationalized hostnames become punycode", async () => {
    const { model } = canonicalize("https://пример.рф/x")
    expect(model.hostname).toBe("xn--e1afmkfd.xn--p1ai")
  })

  test("unicode path segments decode to raw model strings", async () => {
...
describe("aggressive normalization", async () => {
  test("strips tracking parameters when enabled", async () => {
  ...
  test("keeps tracking parameters by default", async () => {
